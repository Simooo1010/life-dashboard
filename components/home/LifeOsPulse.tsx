  ...
  return (
    <section aria-labelledby="life-os-pulse" className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="rounded-2xl border border-border bg-white dark:bg-surface p-3"><p className="text-2xl font-semibold text-ink">{tomorrowCount}</p><p className="text-2xs text-ink-muted">domani</p></div>
        <div className="rounded-2xl border border-border bg-white dark:bg-surface p-3"><p className="text-2xl font-semibold text-ink">{schoolCount}</p><p className="text-2xs text-ink-muted">scuola aperta</p></div>
      </div>
      {overview.anomalies.length > 0 && <p className="text-xs text-amber-700 dark:text-amber-400">{overview.anomalies[0].message}</p>}
    </section>
  )
}

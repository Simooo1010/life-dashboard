...
export function SystemStatus({ sources, anomalies, pageUrl }: { sources: LifeOsSourceStatus[]; anomalies: LifeOsIssue[]; pageUrl: string }) {
  return (
    <details className="rounded-2xl border border-border bg-white p-5 text-sm">
          ...
          {sources.map((source, index) => {
            ...
            return (
              <div key={`${source.source}:${source.sourceId ?? index}`} className="flex items-start gap-3">
                <Icon size={15} className={source.state === 'available' ? 'mt-0.5 text-green-600' : source.state === 'unavailable' ? 'mt-0.5 text-red-500' : 'mt-0.5 text-amber-600'} />
                <div>

...
export function TodayNow({ items }: { items: OperationalItem[] }) {
  return (
    <section aria-labelledby="life-os-today" className="relative overflow-hidden rounded-3xl border border-blue-100 dark:border-blue-900/40 bg-white dark:bg-surface p-5 shadow-sm md:p-6">
      ...
      <div className="mb-5 flex items-start justify-between gap-4 pl-1">
        ...
        {items.some(item => item.issues.length > 0) && <CircleAlert size={17} className="mt-1 shrink-0 text-amber-600 dark:text-amber-400" aria-label="Ci sono elementi da controllare" />}
      </div>
      {items.length > 0 ? (

...
import { loadCachedSecondBrainPage } from '@/lib/cache/dashboard-data'
...
async function SecondBrainContent() {
  const { result } = await loadCachedSecondBrainPage()
  return (
    <>
...
function SecondBrainFallback() {
  return (
    <div aria-busy="true" aria-label="Connessioni in caricamento" className="space-y-3">
  ...
  return (
    <AppShell>
      <div className="max-w-2xl mx-auto px-4 md:px-8 py-8 space-y-10">
        ...
        <Suspense fallback={<SecondBrainFallback />}>
          <SecondBrainContent />
        </Suspense>
      </div>
    </AppShell>
